...
extern "C" {
    void* hf3fs_iovtable_new(const char* mount_name);
    void hf3fs_iovtable_drop(void* table);
    void* hf3fs_iovtable_add_iov(void* table, const char* key, const char* shm_path, uint32_t uid, uint32_t gid);
    void* hf3fs_iovtable_rm_iov(void* table, const char* key, uint32_t uid, uint32_t gid);
    void* hf3fs_iovtable_lookup_iov(void* table, const char* key, uint32_t uid, uint32_t gid);
    void** hf3fs_iovtable_list_iovs(void* table, uint32_t uid, uint32_t gid, size_t* out_len);
}
...
IovTable::IovTable(const std::string& mountName) {
    rust_ptr_ = hf3fs_iovtable_new(mountName.c_str());
    if (!rust_ptr_) throw std::runtime_error("Failed to create Rust IovTable");
}

IovTable::~IovTable() {
    if (rust_ptr_) {
        hf3fs_iovtable_drop(rust_ptr_);
        rust_ptr_ = nullptr;
    }
}
